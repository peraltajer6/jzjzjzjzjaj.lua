...
-- Update function to sync with shared table
...
sendButton.MouseButton1Click:Connect(function()
    if inputBox.Text ~= "" then
        local msg = {text = Player.Name .. ": " .. inputBox.Text}
        table.insert(sharedMessages,msg)
        updateChat()
        inputBox.Text = ""
    end
end)

-- Periodically sync chat with other script users
game:GetService("RunService").RenderStepped:Connect(updateChat)
